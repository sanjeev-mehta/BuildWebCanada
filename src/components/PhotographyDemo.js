...
export default function PhotographyDemo() {
  useEffect(() => {
    ...
    const updateScrollMotion = () => {
      ...
      frame = requestAnimationFrame(() => {
        const demo = document.querySelector('.photo-demo');
        if (demo) demo.style.setProperty('--photo-scroll-shift', `${Math.min(window.scrollY * 0.1, 110)}px`);
      });
    };
    window.addEventListener('scroll', updateScrollMotion, { passive: true });
    updateScrollMotion();
    return () => { observer.disconnect(); window.removeEventListener('scroll', updateScrollMotion); cancelAnimationFrame(frame); };
  }, []);
  return <main className="photo-demo">
    ...
    <section className="photo-hero"><img src={hero} alt="Original candlelit wedding editorial" fetchPriority="high" /><div className="photo-veil" /><div className="photo-scroll-orbit" aria-hidden="true"><i /><i /><b>SCROLL TO REMEMBER</b></div><div className="photo-hero-copy"><p>Wedding & editorial photography</p><h1>Feel it<br /><i>all again.</i></h1><a href="#photo-work">View selected work <b>↓</b></a></div><span className="photo-count">01 / 04</span></section>
